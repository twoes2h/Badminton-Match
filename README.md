...
- 用户注册/登录：性别、出生年份、积分、技术等级、角色、黑名单。
  - 房主或管理员可以在房间内手动添加临时成员，默认密码为 `000000`。
  - 临时成员修改密码后转为正式用户；超过 1 个月未修改密码会从房间成员中清理，没有比赛历史的临时账号会一并删除。
  - 登录用户可以在“我的资料”中修改个人信息、头像和密码。
  - 系统按活跃会话统计在线人数，默认普通用户最多 `100` 人同时在线；满员时会限制新登录并提示稍后再试。
- 公告系统：
  - 管理员可以在管理页添加或修改公告。
  - 用户进入首页时每条公告只自动显示一次，也可以随时点“公告”按钮查看。
...
5. 修改 `.env` 里的数据库密码和 `SESSION_SECRET`。如需调整登录容量，可设置 `MAX_ONLINE_USERS`。
...
```bash
ONLINE_TEST_CONCURRENCY=120 ONLINE_TEST_ROUNDS=3 node scripts/online-load-test.js
```

登录容量限制由 `MAX_ONLINE_USERS` 控制，默认值为 `100`；管理员账号不受该限制影响，方便满员时进入后台处理。
